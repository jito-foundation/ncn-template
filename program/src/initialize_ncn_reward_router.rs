...
pub fn process_initialize_ncn_reward_router(
    ...
    if ncn_reward_router_pubkey.ne(ncn_reward_router.key) {
        ...
        msg!("Expected: {}", ncn_reward_router_pubkey);
        msg!("Received: {}", ncn_reward_router.key);
        return Err(ProgramError::InvalidAccountData);
    }
    msg!("✓ PDA validation passed");
    ...
    msg!("✓ NCN Reward Router account created successfully");
    ...
    msg!("✓ Rent transfer completed successfully");

    msg!(
        "🎉 NCN reward router initialization completed successfully for epoch: {}",
        epoch
    );
    Ok(())
}
